...
export function TechExpertise() {
 ...
 whileInView={{ opacity: 1, y: 0 }}
 viewport={{ once: true }}
 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display font-bold text-black tracking-tight"
 >
 ...
 whileInView={{ opacity: 1, y: 0 }}
 viewport={{ once: true }}
 className="max-w-xs text-sm sm:text-base lg:text-lg leading-relaxed text-black/70 sm:text-right"
 >
 {t.techExpertise.subtitle}
 </motion.p>
 </div>

 <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:gap-8 lg:grid-cols-4">
 {EXPERTISE.map((group, i) => (
 <div
 key={group.category}
 className="group relative overflow-hidden rounded-2xl border border-black/10 bg-[#F8F9FA] p-6 sm:p-8 lg:p-10 transition-all hover:border-black/20 hover:shadow-md"
 >
 <div className="mb-3 sm:mb-4">
 <p className="font-mono text-xs font-bold uppercase tracking-widest text-black/40 mb-1">
 {String(i + 1).padStart(2, "0")}
 </p>
 <h3 className="font-display text-base sm:text-lg lg:text-xl font-bold uppercase tracking-tight text-black">
 {group.category}
 </h3>
 <p className="mt-2 text-sm sm:text-base leading-relaxed text-black/70">
 {group.description}
 </p>
 </div>
